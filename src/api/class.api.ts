...
export const createClass = (data: Pick<IClass, 'name' | 'teacherId' | 'courseId' | 'blockId'>) => http.post<any>(`/class`, data);
export const editClass = (id: number, data: { id: number, name: string, teacherId: number, courseId: number, blockId: number }) => http.patch<IClass>(`/class/${id}`, data);
...
export const removeStudentInClass = (data: Pick<IClassToStudent, 'classId' | 'userId'>) => http.post<any>(`/class-user/remove-student`, data);
